...
module.exports = Mackerel = (function() {
  ...
  Mackerel.prototype.postMetric = function(data, cb) {
    ...
    if (!(data instanceof Array)) {
      return deferred(new Mackerel.NoArrayDataError("postMetric"));
    }
    return this.api("/tsdb", "POST", data, cb);
  };

  Mackerel.prototype.postServiceMetric = function(service, data, cb) {
    if (!data) {
      return deferred(new Mackerel.NoDataError("postServiceMetric"));
    }
    if (!(data instanceof Array)) {
      return deferred(new Mackerel.NoArrayDataError("postServiceMetric"));
    }
    return this.api("/services/" + service + "/tsdb", "POST", data, cb);
  };
...
Mackerel.NoArrayDataError = (function(_super) {
  __extends(NoArrayDataError, _super);

  function NoArrayDataError(api) {
    this.message = "api `" + api + "` requires Array `data`";
    NoArrayDataError.__super__.constructor.call(this, this.message);
  }

  return NoArrayDataError;
